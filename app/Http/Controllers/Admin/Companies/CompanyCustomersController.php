...
{
    //TODO: Companies Users index view
    public function index(Request $request)
    {
        if($request->ajax())
        {
            $users = User::has('companies')->with('companies')->withCount(['transactions' => function($query)
            {
                return $query->where('status', '!=', '');
            }])->where('company_id', '!=', admin_company_id())->where(['user_role' => 3])->orderBy('id', 'DESC')->get();
            return Datatables::of($users)
            ->addIndexColumn()
            ->editColumn('id', 'ID: {{$id}}')
            ->addColumn('payment_status_field', function ($users) {
            ...
            ->addColumn('action', function ($users) {
                ...
                <input type="hidden" name="link" value="'.$users->redirect_url.'">
        ...
        return view('admin.companies.companies-customers.index');
    }
}
