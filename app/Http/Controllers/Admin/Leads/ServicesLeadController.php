...
use App\Models\ServiceLeadLog;
use App\Models\User;
use Carbon\Carbon;
use Illuminate\Database\Eloquent\Builder;

class ServicesLeadController extends Controller
{
    public function index()
    {
        //TODO: Passing Data to yajra Datatables
        if (auth()->user()->role_id == 1) {
            $leads = ServiceLead::with(['service_assigned_to' => function ($query) {
                $query->with('user');
            }])->orderBy('id', 'desc')->get();
        } else {
            $leads = ServiceLead::whereHas('service_assigned_to' , function (Builder $query) {
                $query->where('assigned_to',auth()->user()->id);
            })->with(['service_assigned_to' => function($q){
                $q->with('user');
            }])->orderBy('id', 'desc')->get();
        }
       // return $leads;
      ...
      ->addColumn('assign_lead', function ($leads) {
          ...
          if ($leads->status == 0) {
            ...
            ';
              ...
              $assigned_lead = AssignedServiceLead::where('lead_id', $leads->id)->first();
            // return $assigned_lead;
              if ($assigned_lead) {
                  ...
                  if ($company->role_id == 3) {
                      $html .= $company->name .' | '. $company->companies->company_name;
                      $html .= $company->name .' | '. $company->companies->company_name;
                  } else {
                      $html .= $company->name;
                  }
                  ...
                  if($leads->service_assigned_to !='')
                  {
                    $companies = User::with('companies')->where(['role_id' => $leads->service_assigned_to->user->role_id, 'is_active' => 1, 'is_verified' => 1])->get();
                  }
                  $html .= '
            <input type="hidden" name="lead_id" value="'.$leads->id.'"/>
            <input type="hidden" name="status" value="'.$leads->status.'"/>
            <div class="d-none" id="select_divs" data-id="0">
                ...
                if($leads->service_assigned_to !='')
                {
                 $html .=' <option value="2" '.($leads->service_assigned_to->user->role_id == 2 ? "selected" : "").'>Property Manager/Owner</option>
                    <option value="3" '.($leads->service_assigned_to->user->role_id == 3 ? "selected" : "").'>Agent</option>
                    <option value="4" '.($leads->service_assigned_to->user->role_id == 4 ? "selected" : "").'>Bank</option>
                    <option value="5" '.($leads->service_assigned_to->user->role_id == 5 ? "selected" : "").'>Service Provider</option>
                    <option value="6" '.($leads->service_assigned_to->user->role_id == 6 ? "selected" : "").'>Insurance</option>';
               }
               $html .='</select>
                <select id="company_id" name="company_id" class="">
                <option value="">---select Company---</option>';
                if($leads->service_assigned_to !='')
                {
                  if ($companies->count()) {
                      foreach ($companies as $item) {
                          if ($leads->service_assigned_to->user->role_id == 3) {
                              $html .= '<option value="'.$item->id.'" '.($leads->service_assigned_to->user->id == $item->id ? "selected" : "").'>'.$item->name.' | '.$item->companies->company_name.'</option>';
                          } else {
                              $html .= '<option value="'.$item->id.'" '.($leads->service_assigned_to->user->id == $item->id ? "selected" : "").'>'.$item->company_name.'</option>';
                          }
                      }
                  }
                }
      ...
      ->addColumn('action', function ($leads) {
          return '
              <input type="hidden" name="status" value="'.$leads->status.'">
              <a href="'.route('manage-service-leads.edit', $leads->id).'" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-1" data-toggle="tooltip" data-theme="dark" title="Edit">
      ...
      ->make(true);
        ...
        return view('admin.services-leads.index');
    }

    public function get_companies(Request $request)
    {
        $companies = User::with('companies')->where(['role_id' => $request->role_id, 'is_active' => 1, 'is_verified' => 1])->get();
        $option = '<option value="">--select company---</option>';

        if ($companies->count()) {
            foreach ($companies as $item) {
                if ($request->role_id == 3) {
                    $option .= '<option value="'.$item->id.'">'.$item->name.' | '.$item->companies->company_name.'</option>';
                } else {
    ...
    {
        /**Assign Lead */
        $checkLeadAssigned = AssignedServiceLead::where(['assigned_to' => $request->company_id, 'lead_id' => $request->lead_id])->first();
        $lead_data = ServiceLead::where('id', $request->lead_id)->first();
        ...
        if (!is_null($checkLeadAssigned) && $request->company_id != $checkLeadAssigned->id) {
            ...
            /**Updating Lead Status */
            ServiceLead::where('id', $request->lead_id)->update(['status' => $request->status]);
    ...
    {
        $lead = ServiceLead::find($request->id);
    ...
    {
        $lead = ServiceLead::with('requester')->with('service_lead_deatils')->with(['service_assigned_to' => function ($query) {
            $query->with(['user' => function ($query) {
                $query->with('role', 'companies');
            }]);
        }])->with(['service_logs' => function ($query) {
            return $query->with('user')->orderBy('id', 'desc');
        }])->where('id', $id)->first();

        $property = Service::where(['lang_id' => 1, 'id' => $lead->property_id])->first();
        // return $lead;
        return view('admin.services-leads.edit', compact('lead', 'property'));
    }
