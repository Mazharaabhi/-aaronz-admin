...
class DocumentTypeController extends Controller
{
    ...
    {
        //return 'hello';
        $units =DocumentType::orderBy('id', 'DESC')->get();
       // return $units;
       if($request->ajax())
       {
        ...
        ->addColumn('action', function ($units) {
            return '
            <input type="hidden" name="status" value="'.$units->status.'">
            <a href="'.route("admin.settings.document-types.edit", ["id" => $units->id]).'" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-1" data-toggle="tooltip" data-theme="dark" title="Edit">
        ...
        return view('admin.settings.document-types.index', compact('units'));
    }
    //TODO:loading create view
    public function create()
    {
        return view('admin.settings.document-types.create');
    }
     //TODO:for create new DocumentType
     public function create_process(Request $request){
        //checking DocumentType name alrady exists or not
        $check = DocumentType::where('name', $request->title_english)->first();
        if(!is_null($check)){
            return 'exits';
        }
        //creating new DocumentType
        $units = new DocumentType;
    ...
    {
        $unit = DocumentType::where('id',$id)->first();
        return view('admin.settings.document-types.edit', compact('unit'));
    }
    //TODO: EDIT PROCESS START HERE
     public function edit_process(Request $request)
     {
         $units=DocumentType::all();
         foreach($units as $lang)
         {
             //for duplicate DocumentType Name
        ...
        //edit unit
        $unit = DocumentType::find($request->id);
        $unit->name = $request->title_english;
        $unit->update();
        return 'true';
    }
    //TODO:for delete DocumentType
    public function delete(Request $request)
    {
       // return $request->all();
       $lang_del= DocumentType::where('id', $request->id)->delete();
    ...
    //    }
     ...
     {
        $lang = DocumentType::where('id', $request->id)->first();
        if ($lang->status == 1) {
             DocumentType::where('id', $request->id)->update(['status' => 0]);
            //  menu::where('lang_id', $request->id)->update(['status' => 0]);
        } else {
            DocumentType::where('id', $request->id)->update(['status' => 1]);
    ...
    public function make_default(Request $request){
        DocumentType::where(['is_default' => 1])->update(['is_default' => 0]);

        DocumentType::where('id', $request->id)->update(['is_default' => 1, 'status' => 1]);
    }
