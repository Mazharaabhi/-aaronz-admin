...
class BuildingController extends Controller
{
    ...
    {
        $areas = LocationBuilding::with('location_countries', 'location_states', 'location_areas', 'locations')->orderBy('id', 'DESC')->where(['lang_id' => 1])->get();
        if($request->ajax())
        {
            ...
            ->addColumn('action', function ($areas){
                    return '
                     <a href="'.route('locations.buildings.edit', ['id' => $areas->id]).'" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-1" data-toggle="tooltip" data-theme="dark" title="Edit">
        ...
        return view('locations.buildings.index');
    }
    ...
    public function create(){
        ...
        return view('locations.buildings.create',compact('languages', 'location_countries'));
    }
    ...
    public function create_process(Request $request){
         ...
         $CheckLocationName = LocationBuilding::where([
         ...
         //checking for unique rank name
         $CheckLocationSlug = LocationBuilding::where([
            'slug' => $request->slug,
            'lang_id' => 1,
            'location_country_id' => $request->location_country_id,
            'location_state_id' => $request->location_state_id,
         ...
         $location->location_area_id = $request->location_area_id;
         $location->location_id = $request->location_id;
         $location->latitude = $request->latitude;
         $location->longitude = $request->longitude;
         $location->slug = $request->slug;
         if($request->hasFile('image')){
             $location->image = $request->file('image')->store('LocationImages', 'public');
         }
         $location->lang_id = 1;
         ...
         //UPDATING PARENT ID
         LocationBuilding::where('id',$parent_id)->update(['parent_id'=>$parent_id]);
         //CREATEING OTHER LANGUAGES RECORDS
         for($i=1; $i <count($languages_id) ; $i++ )
         {
             $locat = new LocationBuilding;
             $locat->name = $titles[$i];
             ...
             $locat->location_area_id = $request->location_area_id;
             ...
             if($request->hasFile('image')){
                $locat->image = $request->file('image')->store('LocationImages', 'public');
             }
    ...
    public function edit($id){
        $languages = Language::where('status',1)->get();
        $rank = LocationBuilding::where('id',$id)->first();
        for($i = 1; $i < count($languages) ; $i++)
        {
            $checkLanguageAmenity = LocationBuilding::where(['parent_id' => $id, 'lang_id' => $languages[$i]['id']])->first();
            if(is_null($checkLanguageAmenity))
            {
                $nav = new LocationBuilding;
                $nav->name = '';
                $nav->lang_id=$languages[$i]['id'];
                $nav->parent_id=$id;
                $nav->location_country_id=$rank->location_country_id;
                $nav->location_state_id=$rank->location_state_id;
                $nav->location_area_id=$rank->location_area_id;
                $nav->location_id=$rank->location_id;
        ...
        $location_countries = LocationCountry::where(['status' => 1, 'lang_id' => 1])->get();
        $storedData = LocationBuilding::where(['parent_id'=>$id])->get();
        return view('locations.buildings.edit', compact('rank','languages','storedData', 'location_countries'));
    }

      //editing new rank here
      public function edit_process(Request $request){
        //   return $request->all();
        //return $request->title_english;
        $languages_id = explode (",",$request->languages_names);
        $titles = explode (",",$request->titles);
        //checking for unique rank name
        $CheckLocationName = LocationBuilding::where([
        ...
        if(!is_null($CheckLocationName)) return 'title';
        ...
        //creating new rank instance here
        $locat = LocationBuilding::where('id',$request->id)->first();
        ...
        for($i=1; $i <count($languages_id) ; $i++ )
        {
            $location = LocationBuilding::where(['lang_id'=>$languages_id[$i],'parent_id'=>$request->id])->first();
    ...
    public function change_status(Request $request){
        $ctype = LocationBuilding::where('id', $request->id)->first();

        if ($ctype->status == 1) {
            LocationBuilding::where('parent_id', $request->id)->update(['status' => 0]);
        } else {
            LocationBuilding::where('parent_id', $request->id)->update(['status' => 1]);
        }
    ...
    {
       $areas = LocationArea::where(['location_state_id' => $request->location_state_id, 'lang_id' => 1])->get();
    ...
    {
       $areas = Location::where(['location_area_id' => $request->location_area_id, 'lang_id' => 1])->get();
