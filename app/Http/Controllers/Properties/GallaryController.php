...
class GallaryController extends Controller
{
    ...
    {
        $views = Gallary::orderBy('id', 'DESC')->where('lang_id',1)->get();
        if($request->ajax())
        {
         ...
         ->addColumn('action', function ($views) {
                 return '
                 <input type="hidden" name="status" value="'.$views->status.'">
                 <a href="'.route("manage-properties.property-settings.gallaries.edit", ["id" => $views->id]).'" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-1" data-toggle="tooltip" data-theme="dark" title="Edit">
        ...
        return view('properties.gallaries.index', compact('views'));
    }

    //loading create view
    public function create(){
        $languages = Language::where('status',1)->get();
        return view('properties.gallaries.create',compact('languages'));
    }

    //creating new Gallary here
    public function create_process(Request $request){
         $lang_ids = explode (",",$request->languages_names);
         $titles = explode (",",$request->titles);
         //checking for unique rank name
         $CheckViewName = Gallary::where('title', $request->title_english)->first();
         ...
         $parent_id =$view->id;
         //UPDATING PARENT ID
         Gallary::where('id',$parent_id)->update(['parent_id'=>$parent_id]);
    ...
    public function edit($id){
        $languages = Language::where('status',1)->get();
        $view = Gallary::where('id',$id)->first();
        for($i = 1; $i < count($languages) ; $i++)
        {
            $checkLanguageView = Gallary::where(['parent_id' => $id, 'lang_id' => $languages[$i]['id']])->first();
        ...
        $storedData = Gallary::where(['parent_id'=>$id])->get();
        return view('properties.gallaries.edit', compact('view','languages','storedData'));
    }

      //editing new Gallary here
      public function edit_process(Request $request){
        ...
        //Upadating new Gallary here
        $ame = Gallary::find($request->id);
        $ame->title = $request->title_english;
        $ame->update();
        for($i=1; $i <count($lang_ids) ; $i++ )
        {
            $amenity = Gallary::where(['lang_id'=>$lang_ids[$i],'parent_id'=>$request->id])->first();
    ...
    {
        $view = Gallary::where('id', $request->id)->first();
        $active='';
        if($view->status == 1)
        {
            $active = 0;
        ...
        Gallary::where('parent_id' , $request->id)->update(['status' => $active]);
     ...
     {
        Gallary::where('parent_id', $request->id)->delete();
