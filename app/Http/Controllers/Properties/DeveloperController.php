...
class DeveloperController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->ajax())
        {
            $amenities = Developer::orderBy('id', 'DESC')->where('lang_id',1)->get();
            ...
            ->addColumn('action', function ($amenities){
                    return '
                     <a href="'.route('manage-properties.developers.edit', ['id' => $amenities->id]).'" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-1" data-toggle="tooltip" data-theme="dark" title="Edit">
        ...
        return view('properties.developers.index');
    }
    ...
    public function create(){
        ...
        return view('properties.developers.create',compact('languages', 'icons'));
    }
    ...
    public function create_process(Request $request){
         ...
         $CheckDeveloperName = Developer::where('name', $request->title_english)->first();

         if(!is_null($CheckDeveloperName)){
             return 'title';
         }
         //creating new rank instance here
         $amenity = new Developer;
         $amenity->name = $request->title_english;

         if($request->hasFile('image')){
            $file=$request->file('image')->store('DeveloperImages', 's3');
    ...
    public function edit($id){
        $languages = Language::where('status',1)->get();
        $rank = Developer::where('id',$id)->first();
        for($i = 1; $i < count($languages) ; $i++)
        {
            $checkLanguageDeveloper = Developer::where(['parent_id' => $id, 'lang_id' => $languages[$i]['id']])->first();
            if(is_null($checkLanguageDeveloper))
            {
                $nav = new Developer;
                $nav->name = '';
        ...
        $storedData = Developer::where(['parent_id'=>$id])->get();
        return view('properties.developers.edit', compact('rank','languages','storedData'));
    }
      ...
      public function edit_process(Request $request){
        ...
        for($i=1; $i <count($languages_id) ; $i++ )
        {
            $amenity = Developer::where(['lang_id'=>$languages_id[$i],'parent_id'=>$request->id])->first();
    ...
    public function change_status(Request $request){
        $ctype = Developer::where('id', $request->id)->first();

        if ($ctype->status == 1) {
            Developer::where('parent_id', $request->id)->update(['status' => 0]);
        } else {
            Developer::where('parent_id', $request->id)->update(['status' => 1]);
        }

        echo $ctype->status;
    }

     //TODO:for delete Developer
     public function delete(Request $request)
     {
          Developer::where('parent_id', $request->id)->delete();
