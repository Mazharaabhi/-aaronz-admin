...
class PaymentMethodController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->ajax())
        {
            $amenities = PaymentMethod::orderBy('id', 'DESC')->where('lang_id',1)->get();
            ...
            ->addColumn('action', function ($amenities){
                    return '
                     <a href="'.route('manage-properties.payment-methods.edit', ['id' => $amenities->id]).'" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-1" data-toggle="tooltip" data-theme="dark" title="Edit">
        ...
        return view('properties.payment-methods.index');
    }
    ...
    public function create(){
        ...
        return view('properties.payment-methods.create',compact('languages', 'icons'));
    }
    ...
    public function create_process(Request $request){
         ...
         $CheckPaymentMethodName = PaymentMethod::where('name', $request->title_english)->first();

         if(!is_null($CheckPaymentMethodName)){
             return 'title';
         }
         //creating new rank instance here
         $amenity = new PaymentMethod;
         $amenity->name = $request->title_english;
         if($request->hasFile('image'))
         {
            $amenity->image = $request->file('image')->store('PaymentMethodImages', 'public');
         }
         $amenity->lang_id = 1;
         $amenity->parent_id = 0;
         $amenity->save();
         ...
         //UPDATING PARENT ID
         PaymentMethod::where('id',$parent_id)->update(['parent_id'=>$parent_id]);
         //CREATEING OTHER LANGUAGES RECORDS
         for($i=1; $i <count($languages_id) ; $i++ )
         {
             $ame = new PaymentMethod;
             $ame->name = $titles[$i];
             if($request->hasFile('image'))
                {
    ...
    public function edit($id){
        $languages = Language::where('status',1)->get();
        $rank = PaymentMethod::where('id',$id)->first();
        for($i = 1; $i < count($languages) ; $i++)
        {
            $checkLanguagePaymentMethod = PaymentMethod::where(['parent_id' => $id, 'lang_id' => $languages[$i]['id']])->first();
            if(is_null($checkLanguagePaymentMethod))
            {
                $nav = new PaymentMethod;
                $nav->name = '';
        ...
        $storedData = PaymentMethod::where(['parent_id'=>$id])->get();
        return view('properties.payment-methods.edit', compact('rank','languages','storedData'));
    }
      ...
      public function edit_process(Request $request){
        ...
        if(!is_null($CheckPaymentMethodName)){
            if($CheckPaymentMethodName->id != $request->id){
            return 'title';
            }
        }
        //creating new rank instance here
        $ame = PaymentMethod::find($request->id);
        $ame->name = $request->title_english;
             if($request->hasFile('image'))
                {
                    $ame->image = $request->file('image')->store('PaymentMethodImages', 'public');
                }
        $ame->update();
        for($i=1; $i <count($languages_id) ; $i++ )
        {
            $amenity = PaymentMethod::where(['lang_id'=>$languages_id[$i],'parent_id'=>$request->id])->first();
    ...
    public function change_status(Request $request){
        $ctype = PaymentMethod::where('id', $request->id)->first();

        if ($ctype->status == 1) {
            PaymentMethod::where('parent_id', $request->id)->update(['status' => 0]);
        } else {
            PaymentMethod::where('parent_id', $request->id)->update(['status' => 1]);
        }

        echo $ctype->status;
    }

     //TODO:for delete PaymentMethod
     public function delete(Request $request)
     {
          PaymentMethod::where('parent_id', $request->id)->delete();
