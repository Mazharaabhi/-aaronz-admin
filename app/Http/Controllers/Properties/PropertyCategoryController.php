...
class PropertyCategoryController extends Controller
{
    ...
    {
        $views = PropertyCategory::with('types')->orderBy('id', 'ASC')->where(['lang_id' => 1, 'level' => 1])->get();

        if($request->ajax())
        {
         return Datatables::of($views)
         ->addIndexColumn()
         ->addColumn('is_commercial', function($is_com) {
           if($is_com->is_commercial == 0){
               return 'Commercial';
           } else if($is_com->is_commercial == 1){
            return 'Residential';
           }
         })
         ...
         ->addColumn('action', function ($views) {
                 ...
                 return '
                 <input type="hidden" name="status" value="'.$views->status.'">
                 <a href="'.route("manage-properties.property-settings.categories.edit", ["id" => $views->id]).'" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-1" data-toggle="tooltip" data-theme="dark" title="Edit">
        ...
        return view('properties.categories.index', compact('views'));
    }
    ...
    public function create(){
        ...
        return view('properties.categories.create',compact('languages', 'menus', 'property_includes'));
    }
    ...
    public function create_process(Request $request){
         ...
         $category->is_commercial = $request->is_commercial;
         $category->slug = $request->slug;
         $category->property_includes = $request->property_includes;
         $category->level = 1;
         $category->lang_id = 1;
         ...
         for($i=1; $i <count($views_id) ; $i++ )
         {
             ...
             $cat->is_commercial = $request->is_commercial;
             $cat->slug = $request->slug;
             $cat->property_includes = $request->property_includes;
             $cat->level = 1;
    ...
    public function edit($id){
        ...
        for($i = 1; $i < count($languages) ; $i++)
        {
            ...
            if(is_null($checkLanguageView))
            {
                ...
                $nav->is_commercial=$view->is_commercial;

                $nav->property_includes=$view->property_includes;
                $nav->slug=$view->slug;
                $nav->level=1;
                $nav->parent_id=$id;
                $nav->save();
        ...
        $storedData = PropertyCategory::where(['parent_id'=>$id])->get();
        $property_includes = DB::table('property_includes')->get(['id','name']);
        $includes = $storedData[0]->property_includes ? explode(',', $storedData[0]->property_includes) : [];
        return view('properties.categories.edit', compact('view','languages','storedData', 'menus', 'property_includes', 'includes'));
    }
