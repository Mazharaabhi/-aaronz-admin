        ...
        columns: [
         ...
         $('body').delegate('#delete_language','click',function(){
              ...
              $.confirm({
                    ...
                    buttons: {
                        ...
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('location.areas.delete') }}",
                                    method:"POST",
           ...
           $('body').delegate('#change_status','click',function(){
              ...
              const option = $(this);
              $.ajax({
                  url:"{{ route('location.areas.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
           ...
           $('body').delegate('#is_show','click',function(){
              const id = $(this).parent().parent().parent().find('input[name="id"]').val();
              let is_show = $(this).parent().parent().parent().find('input[name="is_show"]').val();

              is_show = is_show == 1 ? 0 : 1;

              $.ajax({
                  url:"{{ route('location.areas.is_show') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id, is_show},
                  success:function(res){
                    DataTable.ajax.reload();
                  },error:function(xhr){
