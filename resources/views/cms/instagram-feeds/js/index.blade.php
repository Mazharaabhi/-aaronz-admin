         ...
         $('body').delegate('#delete_language','click',function(){
            // return console.log('hello');
              ...
              $.confirm({
                    ...
                    content: 'Are You Sure you want to delete this Instagram Feed? ',
                    boxWidth: '20%',
                    buttons: {
                        cancel: function () {
                        },
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('cms.instagram-feed.delete') }}",
                                    method:"POST",
                                    data : {"_token":"{{ csrf_token() }}",id},
                                    success:function(res)
                                    {
                                        DataTable.ajax.reload();
                                        $.alert('Instagram Feed Deleted Successfully!');;
                                    },
                                    error:function(xhr)
                                    {
           ...
           $('body').delegate('#change_status','click',function(){
              ...
              const option = $(this);
              $.ajax({
                  url:"{{ route('cms.instagram-feed.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
