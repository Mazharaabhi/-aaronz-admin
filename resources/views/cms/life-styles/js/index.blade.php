...
<script>
    ...
    $(document).ready(function(){
        ...
        ajax: "{{ route('cms.life-styles.index')  }}",
        columns: [
            { data: 'DT_RowIndex' },
            { data: 'title' },
            { data: 'sub_title' },
         ...
         $('body').delegate('#delete_language','click',function(){
            // return console.log('hello');
              ...
              $.confirm({
                    ...
                    content: 'Are You Sure you want to delete this This Lifestyle? ',
                    boxWidth: '20%',
                    buttons: {
                        cancel: function () {
                        },
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('cms.life-styles.delete') }}",
                                    method:"POST",
           ...
           $('body').delegate('#change_status','click',function(){
              ...
              const option = $(this);
              $.ajax({
                  url:"{{ route('cms.life-styles.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
