            ...
            if(!title){
              ...
              return  $('#lifestyle_desc').focus();
                ...
                for(var count = 0; count<areas.length; count++){
                 ...
                 $.ajax({
                     url:"{{ route('cms.life-styles.edit-process') }}",
                     method:"POST",
                     data:formData,
                     contentType:false,
                     processData:false,
                     ...
                     beforeSend:function()
                    {
                        $('#edit_lifestyle').html(`${save_icon} @lang('translation.please_wait')`);
                        $('#edit_lifestyle').attr('class',`${btn_cherwell } btn-block  ${spinner}`);
                        $('#edit_lifestyle').attr('disabled',true);
                    },
                    complete:function()
                    {
                        $('#edit_lifestyle').html(`${save_icon} @lang('translation.save')`);
                        $('#edit_lifestyle').attr('class',`${btn_cherwell } btn-block`);
                        $('#edit_lifestyle').removeAttr('disabled');
                    },
                     success:function(res){
                       //  return console.log(res);
                       if(res == "true"){
                        ToastSuccess("Lifestyle Updated Successfully");
                       } else if(res == 'slug'){
