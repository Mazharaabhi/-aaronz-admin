...
$(document).ready(function(){
   ...
   MakeMenuActive('#property-managers', '#property-manager');
    ...
    $('#package_id').change(function()
    {
        ...
        {
            $.ajax({
                url:"{{ route('admin.companies.manage-companies.get-package') }}",
                method:"GET",
                data:{package_id},
                success:function(res)
                {
                    $('#dynamic_fields').html(res);
                },
                error:function(xhr)
                {
    ...
    $('#add').click(function(){
            ...
            else
            {
                ...
                $.ajax({
                    url:"{{ route('property-manager.check-unique-bank-numbers') }}",
                    method:"POST",
   ...
   $('#save').click(function()
   {
    ...
    //return console.log(area_id);
       ...
       else
       {
           ...
           formData.append('active_sms_id', 0);
           formData.append('_token', _token);

           //TODO: Seding Ajax Requrest for creating navbar menu
           $.ajax({
               url:"{{ route('property-manager.create-process') }}",
               method:"POST",
               data:formData,
               contentType:false,
               processData:false,
