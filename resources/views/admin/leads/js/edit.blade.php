<script>
    $(document).ready(function(){
       ...
       $('#save').click(function()
       {
           ...
           else
           {
               ...
               $.ajax({
                   url:"{{ route('manage-leads.update-lead') }}",
                   method:"POST",
                   data:formData,
                   contentType:false,
                   processData:false,
       ...
       $('#add_comment').click(function(){
            ...
            $('#comment_error').html('');
            $.ajax({
                url:"{{ route('manage-leads.add-comment') }}",
                method:"POST",
       ...
       $('body').delegate('#lead_for', 'change', function(){
        ...
        {
            $.ajax({
                url:"{{ route('manage-leads.get-companies') }}",
                method:"GET",
                data:{role_id},
                success:function(res){
                    console.log(res)
    ...
    $('#company_id').change(function(){
        ...
        if(company_id != ""){
            ...
            $.confirm({
                    title: 'Confirm!',
                    content: 'Assign Lead To This Company|Agent ?.',
                    boxWidth: '20%',
                    buttons: {
                        cancel: function () {
                        },
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('manage-leads.assign-lead') }}",
                                    method:"POST",
    ...
    /**Chaning Lead Status*/
        $.ajax({
            url:"{{ route('manage-leads.update-lead-status') }}",
            method:"POST",
