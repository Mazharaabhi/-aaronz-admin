...
<script>
    ...
    $(document).ready(function(){
        ...
        ajax: "{{ route('admin.settings.sizes.index')  }}",
        columns: [
            { data: 'DT_RowIndex' },
            { data: 'size' },
            { data: 'decimal_size' },
         ...
         $('body').delegate('#delete_unit','click',function(){
              ...
              $.confirm({
                    ...
                    buttons: {
                        ...
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('admin.settings.sizes.delete') }}",
                                    method:"POST",
           ...
           $('body').delegate('#change_status','click',function(){
              ...
              $.ajax({
                  url:"{{ route('admin.settings.sizes.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
           ...
           $('body').delegate('#make_default','click',function(){
              ...
              const option = $(this);
              $.ajax({
                  url:"{{ route('admin.settings.sizes.make-default') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
