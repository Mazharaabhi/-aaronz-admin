...
<div class="content d-flex flex-column flex-column-fluid">
                        ...
                        <fieldset>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="daily_charges" id="daily_charges" class="form-control" placeholder="0.00" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*?)\..*/g, '$1');">
                                    <span class="text-danger" id="daily_charges_error"></span>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="">Service Charges (Hourly)</label>
                                    <input type="text" name="hourly_charges" id="hourly_charges" class="form-control" placeholder="0.00" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*?)\..*/g, '$1');">
                                    ...
                                    <select name="location_id" id="location_id" class="form-control" multiple>
                                        <option value=""></option>
                                                ...
                                                @forelse ($location_country->location_states as $state)
                                                    <option value="{{ '0.'.$state->id }}">{{ $state->name }}</option>
                                                    {{-- For Area --}}
                                                    @forelse ($state->location_areas as $area)
                                                        <option value="{{ '1.'.$area->id }}">{{ $area->name }}, {{ $state->name }}</option>
                                                        {{-- For Locations --}}
                                                        @forelse ($area->locations as $location)
                                                            <option value="{{ '2.'.$location->id }}">{{ $location->name }}, {{ $area->name }}, {{ $state->name }}</option>
                                                            {{-- For Locations --}}
                                                            @forelse ($location->buildings as $building)
                                                            <option value="{{ '3.'.$building->id }}">{{ $building->name }}, {{ $location->name }}, {{ $area->name }}, {{ $state->name }}</option>
                                                            @empty
                                ...
                                <div class="col-md-12 mb-3 {{ $sub->id != 1 ? 'd-none' : '' }}" id="description_div_{{ $sub->id }}">
                                  ...
                                  <textarea name="descriptions[]" id="description_english_{{ $sub->id }}" @if($sub->direction == 'Right') dir="rtl" @endif cols="30" rows="10" class="form-control"></textarea>
    ...
    <!--end::Entry-->
@include('services.list-service.js.create');
@endsection
