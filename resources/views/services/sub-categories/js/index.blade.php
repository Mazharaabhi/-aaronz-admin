...
<script>
    ...
    $(document).ready(function(){
        ...
        ajax: "{{ route('manage-services.sub-category.index')  }}",
        columns: [
            { data: 'DT_RowIndex' },
            { data: 'name' },
            { data: 'service_categories.name' },
            { data: 'image' },
            { data: 'slug' },
            { data: 'questionnaire' },
            { data: 'status' },
         ...
         $('body').delegate('#delete_language','click',function(){
              ...
              $.confirm({
                    ...
                    buttons: {
                        ...
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('manage-services.sub-category.delete') }}",
                                    method:"POST",
           ...
           $('body').delegate('#change_status','click',function(){
              const id = $(this).attr('data-id');
              $.ajax({
                  url:"{{ route('manage-services.sub-category.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
