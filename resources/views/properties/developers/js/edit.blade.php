            ...
            //applying validations here
             if(!title_english || !$.trim(title_english).length){
                ...
                formData.append('_token',_token);
                 ...
                 $.ajax({
                     url:"{{ route('manage-properties.developers.edit-process') }}",
                     method:"POST",
                     data:formData,
                     contentType:false,
                     processData:false,
                     ...
                     beforeSend:function()
                    {
                        $('#add_developer').html(`${save_icon} @lang('translation.please_wait')`);
                        $('#add_developer').attr('class',`${btn_cherwell } btn-block  ${spinner}`);
                        $('#add_developer').attr('disabled',true);
                    },
                    complete:function()
                    {
                        $('#add_developer').html(`${save_icon} @lang('translation.update')`);
                        $('#add_developer').attr('class',`${btn_cherwell } btn-block`);
                        $('#add_developer').removeAttr('disabled');
                    },
                     success:function(res){
                        //return console.log(res);
                       if(res == "true"){
                        ToastSuccess("Developer Updated Successfully");
                       }else if(res == 'title'){
                        $('#title_english_error').html("This developer title is already exist.");
                        $('#title_english').focus();
                       }
