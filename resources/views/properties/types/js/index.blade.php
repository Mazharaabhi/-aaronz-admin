...
<script>
    ...
    $(document).ready(function(){
        ...
        ajax: "{{ route('manage-properties.property-settings.types.index')  }}",
        columns: [
            { data: 'DT_RowIndex' },
            { data: 'name' },
            { data: 'status' },
         ...
         $('body').delegate('#delete_language','click',function(){
              ...
              $.confirm({
                    ...
                    buttons: {
                        ...
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('manage-properties.property-settings.types.delete') }}",
                                    method:"POST",
           ...
           $('body').delegate('#change_status','click',function(){
              const id = $(this).attr('data-id');
              $.ajax({
                  url:"{{ route('manage-properties.property-settings.types.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
