...
<div class="content d-flex flex-column flex-column-fluid">
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-toolbar">
                           <div class="example-tools justify-content-center">
                               <a href="{{ route('manage-properties.property-settings.gallaries.index') }}" class="btn btn-cherwell float-right"><span class="fa fa-mail-reply"></span> Back</a>
                           </div>
                          </div>
                       </div>
                    <div class="card-body">
                        <fieldset>
                           ...
                           @for ($i=0 ; $i < count($languages) ; $i++)
                            <div class="col-md-12 mb-3 {{ $languages[$i]->id != 1 ? 'd-none' : '' }}" id="div_{{ $languages[$i]->id }}">
                                <label for="title_english">View Title {{ $languages[$i]->name }}</label>
                                <input type="text" name="title_english[]" @if($languages[$i]->direction == 'Right') dir="rtl" @endif id="title_english_{{ $languages[$i]->id }}" value="@if($languages[$i]->id == $storedData[$i]->lang_id) {{ $storedData[$i]->title }} @endif" class="form-control">
    ...
    <!--end::Entry-->
@include('properties.gallaries.js.edit');
@endsection
