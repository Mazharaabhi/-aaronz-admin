...
<script>
    ...
    $(document).ready(function(){
        ...
        ajax: "{{ route('manage-properties.property-settings.gallaries.index')  }}",
        columns: [
            { data: 'DT_RowIndex' },
            { data: 'title' },
            { data: 'status' },
         ...
         $('body').delegate('#delete_language','click',function(){
              ...
              $.confirm({
                    ...
                    buttons: {
                        ...
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('manage-properties.property-settings.gallaries.delete') }}",
                                    method:"POST",
           ...
           $('body').delegate('#change_status','click',function(){
              const id = $(this).attr('data-id');
              $.ajax({
                  url:"{{ route('manage-properties.property-settings.gallaries.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
