...
<script>
    let mymap;
    let marker;
    var g_lat = "{{ isset($lat) ? $lat : ''  }}";
    var g_lng = "{{ isset($lng) ? $lng : ''  }}";
    let is_from_map = 0;
    let level_id = "{{ isset($level_id) ? $level_id : ''  }}";
    let area_id  = "{{ isset($arid) ? $arid : ''  }}";
...
function initMap(lat=g_lat,lng=g_lng) {

   // alert(`lat: ${lat},lng: ${lat}`);
    var markers = [
        {
          //  "title": 'Lahore',
            "lat": lat,
