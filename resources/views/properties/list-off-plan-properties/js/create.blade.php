...
<script>
    $(document).ready(function() {
        ...
        //for image preview
        function readURbroucher(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    $('#blah-broucher').attr('src', e.target.result);
                    $('#blah-broucher').attr('class', 'd-block')
                }
                reader.readAsDataURL(input.files[0]); // convert to base64 string
            }
        }
        $("#fileupload-broucher").change(function() {
            readURbroucher(this);
        });
        ...
        $('#create_property').click(function() {
            ...
            let is_basic = $("input[name='is_basic']:checked").val();
            ...
            // }
                ...
                //***MAKING ARRAY OF DESC START HERE****//
                var two_d = document.getElementById('fileupload-2d').files[0];
                var three_d = document.getElementById('fileupload-3d').files[0];
                var signature_image = document.getElementById('signature-image').files[0];
                var signature_section_two_image = document.getElementById('signature_section_two_image')
                    .files[0];
                var signature_section_three_image = document.getElementById('signature_section_three_image').files[0];
