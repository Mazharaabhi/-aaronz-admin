...
<script>
    ...
    $(document).ready(function(){
        ...
        MakeMenuActive('#c_properties_1', '#payment_methods', '#cms_anchor');
        //TODO: Getting Data and passing into yajra datatables
        var DataTable = $('#users-table').DataTable({
        processing: true,
        serverSide: true,
                dom: '<"top"f><"responsivetb"rt><"bottom"ilp><"clear">',
        ajax: "{{ route('manage-properties.payment-methods.index')  }}",
        columns: [
            { data: 'DT_RowIndex' },
            { data: 'name' },
            { data: 'image' },
         ...
         $('body').delegate('#delete_language','click',function(){
              ...
              $.confirm({
                    ...
                    buttons: {
                        ...
                        confirm: {
                            ...
                            action: function(){
                                $.ajax({
                                    url:"{{ route('manage-properties.payment-methods.delete') }}",
                                    method:"POST",
           ...
           $('body').delegate('#change_status','click',function(){
              ...
              const option = $(this);
              $.ajax({
                  url:"{{ route('manage-properties.payment-methods.status') }}",
                  method:"POST",
                  data : {"_token":"{{ csrf_token() }}",id},
                  success:function(res){
                    DataTable.ajax.reload();
